...
- [ ] Flutter 3.24.0 minimum-SDK and downgraded-dependency checks pass.
...
4. The tag workflow verifies the tag against package metadata and changelog.
5. Approve the `pub-dev` Environment deployment as GitHub user `ntfnd404`.
6. OIDC publishes without a persistent token.
7. A transient infrastructure failure may rerun the unchanged job. A code or
